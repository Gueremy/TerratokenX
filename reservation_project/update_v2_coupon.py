...
<script>
    function rwaWizard() {
        return {
            ...
            calculateTotal() {
                const subtotal = this.projectPrice * this.qty;
                if (this.discount > 0) {
            ...
            handleSubmit() {
                // Ensure metodo_pago is set in the form
                const form = document.getElementById('rwaForm');
                form.submit();
            }
        }
    }
...
print("File updated: coupon logic added, footer/risks/banner removed, payment submission fixed.")
