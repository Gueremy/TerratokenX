...
html_content = """{% extends 'booking/base.html' %}
{% load static %}
{% load humanize %}
            ...
            <form method="post" id="rwaForm" @submit.prevent="handleSubmit">
                ...
                <div x-show="step === 1" x-cloak x-transition>
                    <div class="asset-card shadow-2xl">
                        <div class="asset-card-header d-flex justify-content-between align-items-center">
                            ...
                            <div x-show="availableProjects.length > 1" x-cloak>
                                <select class="form-select-custom py-1 px-2 small" x-model="selectedProjectId"
                        ...
                        <div class="p-0">
                            ...
                            <div class="asset-info-grid">
                                <div class="info-box">
                                    ...
                                    <div class="info-value text-white">$<span
                                            x-text="formatNumber(activeProject.precio)"></span> USD</div>
                                </div>
                                <div class="info-box">
                            ...
                            <!-- Qty Selector con Validación KYC -->
                            <div class="mt-4 px-4 text-center">
                                <p class="text-muted small mb-2 text-uppercase fw-bold tracking-widest"
                                    style="font-size: 0.6rem;">¿Cuántos tokens deseas adquirir?</p>
                            </div>
                            <div class="qty-selector" :class="isOverLimit ? 'border-danger' : ''">
                                <button type="button" class="qty-btn" @click="changeQty(-1)"><i
                                        class="fas fa-minus"></i></button>
                                <input type="number" name="cantidad_tokens" x-model="qty" @input="validateQty()"
                                    class="qty-input">
                                <button type="button" class="qty-btn" @click="changeQty(1)" :disabled="qty >= maxQty"><i
                                        class="fas fa-plus"></i></button>
                            </div>
                            
                            <!-- Mensaje de Límite KYC -->
                            <div class="text-center mb-3">
                                <div x-show="kycRemaining < 1000000" x-cloak>
                                    <p class="x-small text-muted mb-1">
                                        Tu límite disponible: <span class="text-white font-bold">$<span x-text="formatNumber(kycRemaining)"></span> USD</span>
                                        (<span x-text="kycTier"></span>)
                                    </p>
                                </div>
                                <div x-show="isOverLimit" x-cloak class="text-danger x-small font-bold animate-pulse">
                                    ⛔ Excedes tu cupo. Máximo <span x-text="maxQty"></span> tokens.
                                    <a href="/verification/" class="text-gold underline ms-1">Verificarme ahora</a>
                                </div>
                            </div>
                            ...
                            <div class="coupon-box">
                                ...
                                <div class="d-flex gap-2">
                                    ...
                                    <button type="button" @click="validateCoupon()"
                                        ...
                                        <span x-show="!isCheckingCoupon" x-cloak>VALIDAR</span>
                                        <span x-show="isCheckingCoupon" x-cloak class="spinner-border spinner-border-sm"></span>
                                    </button>
                                </div>
                                <div x-show="couponMessage" x-cloak class="mt-2 small"
                                    :class="couponValid ? 'text-success' : 'text-danger'" x-text="couponMessage"></div>
                            </div>

                            <div class="total-bar">
                                <div class="d-flex flex-column">
                                    <span class="text-muted fw-bold small text-uppercase"
                                        style="letter-spacing: 1px;">Total a Pagar:</span>
                                    <span x-show="discount > 0" x-cloak class="text-success x-small fw-bold">DESCUENTO
                                        APLICADO</span>
                                </div>
                                <span class="h3 text-gold m-0 font-bold"
                                    x-text="'$' + formatNumber(total) + ' USD'"></span>
                            </div>

                            <button type="button" class="btn-gold shadow-lg" @click="step = 2"
                                :disabled="!isStep1Valid || isOverLimit">
                        ...
                        <div class="d-flex gap-3">
                            ...
                            <div>
                                ...
                                <p class="small text-muted mb-0" style="line-height: 1.5;">
                                    Este activo digital es un <strong>Token de Activo (Security Token)</strong>. Su
                                    valor está vinculado exclusivamente al rendimiento inmobiliario del proyecto.
                                </p>
                            </div>
                        </div>
                    </div>
                ...
                <div x-show="step === 2" x-transition x-cloak>
                    <div class="asset-card">
                        <div class="asset-card-header">
                            <h5 class="m-0 uppercase tracking-widest"><i
                                    class="fas fa-user-shield text-gold me-2"></i>Tus Datos</h5>
                        </div>
                        <div class="p-4 p-md-5">
                            <div class="row g-4">
                                <div class="col-md-6">
                                    <label class="info-label mb-2">NOMBRE COMPLETO</label>
                                    <input type="text" name="nombre" x-model="formData.nombre"
                                        class="form-control-custom w-100" placeholder="Ej: Juan Pérez" required>
                                </div>
                                <div class="col-md-6">
                                    <label class="info-label mb-2">EMAIL</label>
                                    <input type="email" name="correo" x-model="formData.correo"
                                        class="form-control-custom w-100" placeholder="correo@ejemplo.com" required>
                                </div>
                                <div class="col-md-6">
                                    <label class="info-label mb-2">RUT / ID NACIONAL</label>
                                    <input type="text" name="rut" x-model="formData.rut"
                                        class="form-control-custom w-100" placeholder="12.345.678-K" required>
                                </div>
                                <div class="col-md-6">
                                    <label class="info-label mb-2">TELÉFONO</label>
                                    <input type="text" name="telefono" x-model="formData.telefono"
                                        class="form-control-custom w-100" placeholder="+56 9 1234 5678" required>
                                </div>
                                <div class="col-md-12">
                                    <label class="info-label mb-2">DIRECCIÓN (Requerido para el contrato)</label>
                                    <input type="text" name="direccion" x-model="formData.direccion"
                                        class="form-control-custom w-100" placeholder="Calle, Número, Ciudad" required>
                                </div>

                                <!-- Persona Jurídica Toggle -->
                                <div class="col-md-12 mt-3">
                                    <div class="form-check custom-check">
                                        <input class="form-check-input bg-black border-secondary" type="checkbox"
                                            id="es_empresa" name="es_empresa" x-model="isCompany">
                                ...
                                <div class="col-md-12" x-show="isCompany" x-cloak x-transition>
                                    <div
                                        class="row g-4 mt-1 p-3 rounded-lg border border-secondary border-opacity-30 bg-black">
                                        <div class="col-md-6">
                                            <label class="info-label mb-2">RAZÓN SOCIAL</label>
                                            <input type="text" name="razon_social" x-model="formData.razon_social"
                                                class="form-control-custom w-100" placeholder="Mi Empresa S.A.">
                                        </div>
                                        <div class="col-md-6">
                                            <label class="info-label mb-2">RUT EMPRESA</label>
                                            <input type="text" name="rut_empresa" x-model="formData.rut_empresa"
                                                class="form-control-custom w-100" placeholder="76.123.456-7">
                                        </div>
                                        <div class="col-md-12">
                                            <label class="info-label mb-2">CARGO REPRESENTANTE</label>
                                            <input type="text" name="cargo_representante"
                                                x-model="formData.cargo_representante" class="form-control-custom w-100"
                        ...
                        <div class="p-4 d-flex gap-3">
                            <button type="button" class="btn btn-outline-secondary flex-grow-1 p-3 rounded-lg font-bold"
                                @click="step = 1">ATRÁS</button>
                            <button type="button" class="btn-gold flex-grow-1 m-0" @click="validateStep2AndProceed()"
                ...
                <div x-show="step === 3" x-transition x-cloak>
                    <div class="asset-card">
                        <div class="asset-card-header">
                            <h5 class="m-0 uppercase tracking-widest"><i
                                    class="fas fa-file-contract text-gold me-2"></i>Legal & T&C</h5>
                        </div>
                        <div class="p-4 p-md-5 text-center">
                            ...
                            <div class="text-start space-y-4">
                                <div class="form-check mb-4 custom-check">
                                    <input class="form-check-input bg-black border-secondary" type="checkbox" id="terms"
                                        required x-model="tcAccepted">
                                    <label class="form-check-label small text-muted ms-2" for="terms">
                                        He leído y acepto los <a href="#"
                                            class="text-gold text-decoration-none">Términos de Inversión</a> y la <a
                                            href="#" class="text-gold text-decoration-none">Política de Privacidad</a>.
                                    </label>
                                </div>
                                <div class="form-check mb-4 custom-check">
                                    <input class="form-check-input bg-black border-secondary" type="checkbox"
                                        id="eligibility" required x-model="eligibleAccepted">
                        ...
                        <div class="p-4 d-flex gap-3">
                            <button type="button" class="btn btn-outline-secondary flex-grow-1 p-3 rounded-lg font-bold"
                                @click="step = 2">ATRÁS</button>
                            <button type="button" class="btn-gold flex-grow-1 m-0" @click="step = 4"
                                :disabled="!isStep3Valid">CONTINUAR AL PAGO</button>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="asset-card">
                        ...
                        <div class="p-4 p-md-5">
                            <div class="row g-4">
                                <div class="col-md-6">
                                    <label class="w-100 cursor-pointer">
                                        <input type="radio" name="metodo_pago" value="MP" class="d-none"
                                        ...
                                        <div class="p-4 text-center rounded-lg border-2 transition-all h-100 d-flex flex-column align-items-center justify-content-center"
                                            ...
                                            <div class="fw-bold text-white uppercase" style="font-size: 0.8rem;">Mercado
                                                Pago</div>
                                            <div class="x-small text-muted mt-1" style="font-size: 0.6rem;">FIAT
                                                (TARJETAS)</div>
                                        </div>
                                    </label>
                                </div>
                                <div class="col-md-6">
                                    <label class="w-100 cursor-pointer">
                                        ...
                                        <div class="p-4 text-center rounded-lg border-2 transition-all h-100 d-flex flex-column align-items-center justify-content-center"
                                            ...
                                            <div class="fw-bold text-white uppercase" style="font-size: 0.8rem;">
                                                Criptomonedas</div>
                                            <div class="x-small text-muted mt-1" style="font-size: 0.6rem;">USDT / BTC /
                                                ETH</div>
                                        </div>
                                    </label>
                                </div>
                            ...
                            <div class="mt-4 text-center">
                                <p class="text-muted small italic">Estamos trabajando en incorporar más métodos de pago
                                </p>
                            </div>
                        </div>
                        <div class="p-4 d-flex gap-3">
                            <button type="button" class="btn btn-outline-secondary flex-grow-1 p-3 rounded-lg font-bold"
                                @click="step = 3">ATRÁS</button>
...
<script>
    function rwaWizard() {
        return {
            ...
            // KYC DATA SECURE
            kycRemaining: parseFloat("{{ kyc_info.remaining_usd|default:0|stringformat:'f' }}"),
            kycTier: "{{ kyc_info.tier_label|default:'Invitado' }}",
            maxQty: 9999,
            ...
            init() {
                ...
                console.log("KYC LIMIT:", this.kycRemaining);

                if (isNaN(this.kycRemaining)) {
                    this.kycRemaining = 0;
                }
                ...
                // Prioridad: Contexto Django > URL > Primer Dispo
                {% if proyecto %}
            ...
            updateProject() {
                let found = this.availableProjects.find(p => String(p.id) === String(this.selectedProjectId));
                if (!found && this.availableProjects.length > 0) {
                    found = this.availableProjects[0];
                    this.selectedProjectId = found.id;
                }
                this.activeProject = found || {};
                
                // Recalcular MAX QTY basado en precio y saldo
                if (this.activeProject.precio > 0) {
                    this.maxQty = Math.floor(this.kycRemaining / this.activeProject.precio);
                    if (this.maxQty < 0) this.maxQty = 0;
                } else {
                    this.maxQty = 9999;
                }
                
                // Ajustar QTY si excede nuevo max
                if (this.qty > this.maxQty && this.maxQty > 0) {
            ...
            changeQty(delta) {
                ...
                // No permitir subir más allá del máximo (excepto si max es 0 que es bloqueo total)
                if (this.maxQty > 0 && newVal > this.maxQty) newVal = this.maxQty;
            ...
            validateQty() {
                if (this.maxQty > 0 && this.qty > this.maxQty) {
                    this.qty = this.maxQty;
                }
                this.calculateTotal();
            },
            ...
            // Validation Getters
            get isOverLimit() {
                // Doble check del total vs cupo
                return (this.total > this.kycRemaining);
            },
            
            get isStep1Valid() {
                return this.selectedProjectId && this.qty >= 1 && !this.isOverLimit;
            },
...
print("Archivo reservation_form_v2.html restaurado correctamente.")
