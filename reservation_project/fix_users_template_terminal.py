...
file_path = r'c:\Users\Guere\OneDrive\Escritorio\WAS DE PROGRAMACION\adaptar sistemas de reservas joan\chelooficial\reservation_project\booking\templates\booking\admin\users.html'

html_content = """{% extends 'booking/admin/base_admin.html' %}
{% load humanize %}
            ...
            <table class="w-full">
                <thead>
                    <tr class="bg-dark-900/50 border-b border-gray-700 text-left">
                        ...
                        <th class="px-6 py-4 text-xs font-bold text-gold-500 uppercase tracking-wider">Inversión</th>
                ...
                <tbody class="divide-y divide-gray-700/50">
                    ...
                    <tr class="hover:bg-white/5 transition-colors group">
                        ...
                        <td class="px-6 py-4">
                            {% if usuario.total_tokens > 0 %}
                                <div class="flex flex-col">
                                    <span class="text-white font-bold">{{ usuario.total_tokens }} Tokens</span>
                                    <span class="text-[10px] text-gold-500/70 uppercase truncate max-w-[150px]" title="{{ usuario.proyectos_lista }}">
                                        {{ usuario.proyectos_lista }}
                                    </span>
                                </div>
                            {% else %}
                                <span class="text-gray-600 text-xs">Sin inversión</span>
                            {% endif %}
                        </td>
                        <td class="px-6 py-4">
...
print(f"Successfully overwrote {file_path} via terminal script.")
