...
def clean_database():
    print("🧹 Cleaning Database Users...")
    users = User.objects.all()
    count = 0
    for user in users:
        changed = False
        # Clean First Name
        if user.first_name and ('<' in user.first_name or '>' in user.first_name):
            print(f"  - Cleaning first_name for {user.username}: {user.first_name}")
            user.first_name = strip_tags(user.first_name)
            changed = True
        
        # Clean Last Name
        if user.last_name and ('<' in user.last_name or '>' in user.last_name):
            print(f"  - Cleaning last_name for {user.username}: {user.last_name}")
            user.last_name = strip_tags(user.last_name)
            changed = True
            
        # Clean Username (if it has tags)
        if '<' in user.username or '>' in user.username:
            print(f"  - Cleaning username for {user.username}")
    ...
    print(f"✅ Cleaned {count} users.")

def force_fix_templates():
    print("\n🔨 Force Fixing Templates (Overwriting)...")
    
    # 1. Fix base.html (User Dashboard Header)
    base_html_path = r"c:\Users\Guere\OneDrive\Escritorio\WAS DE PROGRAMACION\adaptar sistemas de reservas joan\chelooficial\reservation_project\booking\templates\booking\base.html"
    if os.path.exists(base_html_path):
        with open(base_html_path, 'r', encoding='utf-8') as f:
            content = f.read()
        
        # Apply the fix directly to the content string
        if 'Hola, {{ user.first_name|default:user.username }}' in content:
            print("  - Fixing base.html 'Hola' tag...")
            content = content.replace(
                'Hola, {{ user.first_name|default:user.username }}',
                'Hola, {{ user.first_name|default:user.username|striptags }}'
            )
            with open(base_html_path, 'w', encoding='utf-8') as f:
                f.write(content)
        elif 'Hola, {{ user.first_name|default:user.username|striptags }}' in content:
             print("  - base.html already fixed.")
        else:
            print("  - WARNING: Could not match content in base.html")
    else:
        print(f"  - ERROR: base.html not found at {base_html_path}")

    # 2. Fix admin/users.html (Broken Syntax)
    admin_users_path = r"c:\Users\Guere\OneDrive\Escritorio\WAS DE PROGRAMACION\adaptar sistemas de reservas joan\chelooficial\reservation_project\booking\templates\booking\admin\users.html"
    
    # We will write the CORRECT content directly, ensuring single lines
    correct_admin_users_content = """{% extends 'booking/admin/base_admin.html' %}
{% load humanize %}
...
{% endblock %}"""
    ...
    print("  - Writing correct content to users.html...")
    with open(admin_users_path, 'w', encoding='utf-8') as f:
        f.write(correct_admin_users_content)
